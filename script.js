...
function renderExtraItems(key, containerId) {
  ...
  dynState.extras[key].forEach((item) => {
    ...
    row.innerHTML =
      '<input type="text" placeholder="Nama pos" value="' + escapeAttr(item.label) + '">' +
      '<span class="value-input"><span class="prefix">Rp</span><input type="number" step="10000" value="' + item.amount + '"></span>' +
      '<button class="remove-btn" title="Hapus">&times;</button>';
    const [labelInput, amountWrap, removeBtn] = row.children;
    const amountInput = amountWrap.querySelector('input');
    labelInput.addEventListener('input', () => { item.label = labelInput.value; recalc(); });
    amountInput.addEventListener('input', () => { item.amount = parseFloat(amountInput.value) || 0; recalc(); });
...
function sumExtras(key) {
  return dynState.extras[key].reduce((s, i) => s + (i.amount || 0), 0);
}
...
function recalc() {
  ...
  dynState.customScenarios.forEach((sc) => {
    const extraSum = sc.extras.reduce((s, i) => s + (i.amount || 0), 0);
